    ...
    if selected_folder:
        ...
        if selected_annotation_file:
            ...
            if is_repeated == "是":
                ...
                first_start_col1, first_start_col2 = st.columns([1, 3])
                with first_start_col1:
                    first_start_minutes = st.number_input(
                    "分钟", 
                    min_value=0, 
                    format="%d", 
                    key="first_start_min", 
                    value=int(0 if pd.isna(selected_record.get("first_start_time", 0)) else selected_record.get("first_start_time", 0) // 60)
                )
                with first_start_col2:
                    first_start_seconds = st.number_input(
                    "：秒钟", 
                    min_value=0, 
                    max_value=59, 
                    format="%02d", 
                ...
                first_end_col1, first_end_col2 = st.columns([1, 3])
                with first_end_col1:
                    first_end_minutes = st.number_input(
                        "分钟", 
                        min_value=0, 
                        format="%d", 
                        key="first_end_min", 
                        value=int(0 if pd.isna(selected_record.get("first_end_time", 0)) 
                                  else selected_record.get("first_end_time", 0) // 60)
                    )
                with first_end_col2:
                    first_end_seconds = st.number_input(
                        "：秒钟", 
                        min_value=0, 
                        max_value=59, 
                        format="%02d", 
                        key="first_end_sec", 
                        value=int(0 if pd.isna(selected_record.get("first_end_time", 0)) else selected_record.get("first_end_time", 0) % 60)
                    )
                first_end_time = first_end_minutes * 60 + first_end_seconds
            else:
                first_start_time = None
