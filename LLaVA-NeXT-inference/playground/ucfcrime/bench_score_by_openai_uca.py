...
import os
import logging
from bench_hierarchical_questioning_detailed import  eval_event_description_prompt,eval_event_description_with_classification_prompt,eval_event_description_prompt_uca,eval_event_description_prompt_uca_neg
import json
...
class UCAAnswerDataset:
    def __init__(self,answer_path, ground_truth_path):
        ...
        self.eval_prompts_uca = {
            "event_description_question":eval_event_description_prompt_uca_neg,
            #"event_description_with_classification":eval_event_description_with_classification_prompt
        }
...
def UCVL_scoring(args):
    ...
    model_answer_dataset = UCAAnswerDataset(
    ...
    for idx in range(start_index, len(model_answer_dataset)):
        ...
        scores = {}
        ground_truth_description= '\n'.join(answer_sheet['ground_truth']['sentences'])
        detection_score = 100
        ...
        for question_name,model_answer in answer_sheet['model_answer']['answer'].items():
            if question_name != "event_description_question":
                continue
            eval_prompt_format = model_answer_dataset.eval_prompts_uca.get(question_name,None)
...
if __name__ == "__main__":
    ...
    args.ground_truth_path = "/mnt/lustre/chenhaoran/datasets/UCF-Crime-Train/UCAtxt/UCFCrime_Test.json"
    ...
    for answer_path in answer_paths:
        ...
        log_filename = os.path.join(log_dir, base_filename + '1118.log')
        args.save_score_path = os.path.join(save_dir, base_filename + '_score_uca_noprompt_000002.jsonl')
